...
class VoiceCoach extends Notifier<VoiceCoachState> {
  ...
  /// exercises into an empty session, and announces what's first.
  Future<void> enable() async {
    ...
    if (exercises.isEmpty) {
      ...
      if (planned.isNotEmpty) {
        ...
        await _say('Plan loaded — ${planned.length} exercises. First up: '
            '${planned.first}. Tap the mic and tell me your set.');
        return;
      }
    ...
    await _say('Voice coach on. Current exercise: ${_currentName()}. '
        'Tap the mic and tell me your set.');
  }
  ...
  Future<void> dictate() async {
    ...
    await speech.listen((transcript, isFinal) async {
      ...
      if (isFinal) {
        state = state.copyWith(listening: false);
        await _apply(parseSessionVoiceIntent(transcript));
      }
    });
  }

  Future<void> _apply(SessionVoiceIntent intent) async {
    switch (intent) {
