...
- _2026-06-13 — **Implemented (step 5).** Realization deviations from the design, all justified: (1) **fp32 MoveNet Lightning (192px, ~9.4 MB)** is bundled, not fp16 Thunder — the fp16 export emits garbage under ONNX Runtime's CPU kernels, and fp32 Thunder is 25 MB; fp32 Lightning is CPU-correct and a coarse frame-ratio proxy needs no more. (2) **`Arc<Mutex<Session>>`** (not bare `Arc<Session>`): `ort 2.0-rc.12`'s `Session::run` takes `&mut self`; the model is loaded once and locked per inference inside `spawn_blocking`. (3) **`RankedMatch`** carries `&'static Archetype` (no lifetime parameter) — every call site passes the `'static` `library()`. (4) The AC4 fixture is a **public-domain** full-length front photo (Eugen Sandow, LoC); the real model runs in CI and asserts a plausible ratio. Three behaviour-preserving test corrections the qa could not catch pre-implementation: the core `matching::ids` helper returns `Vec<&'static str>` (archetype ids are `'static`); `pose_onnx`'s `#[ignore]` removed (per the qa step-5 checklist); `health.rs`/`common` gain the `AppState.pose` field (mirroring R-0006's `store`). Gates green: fmt / clippy `-D warnings` / build / test._
